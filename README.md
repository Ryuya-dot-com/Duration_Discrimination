# リスニングタスク3（持続時間弁別）単体版

単体で実施する持続時間弁別タスクです。参加者向け UI ではダミーコード「リスニングタスク3」を表示し、研究者向けには本 README に実タスク名や刺激設定を記載しています。
...
4. 終了後、CSV が自動ダウンロードされる（ファイル名: `<ID>_duration_discrimination.csv`）。
