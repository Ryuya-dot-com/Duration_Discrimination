...
3つの音のうち、1つだけが違う Duration Discrimination Test (持続時間弁別テスト) です。違う音は必ず 1 番目か 3 番目で、参加者は「1」か「3」を答えます。音の長さの差は `Stimuli/1.flac` を基準に、`Stimuli/2.flac` 以降で段階的に変えています。

## 実行方法

- 参加者IDを入力
- 日本語インストラクションを読んで「テストを開始」。
- 3つの音が流れたあとに 1/3 ボタンが押せます。最大 75 試行か「折り返し」が 7 回に達したら終了し、自動で CSV (`<ID>_duration_discrimination.csv`) をダウンロードします。完了画面から再ダウンロードも可能です。
